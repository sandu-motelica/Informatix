...
import { deleteToken, generateTokens, saveToken, updateToken } from '../utils/token.js';
...
export const userLogin = async (req,res)=>{
    try{
        ...
        res.setHeader('Set-Cookie', `refreshToken=${tokens.refreshToken}; Expires=${tokenExpiryDate.toUTCString()}; HttpOnly`);
        ...
        if(e instanceof ApiError) return errorMiddleware(res, e.status, e.message,e.errors);
...
export const userLogout = async (req,res)=>{
    try {
        ...
        res.setHeader('Set-Cookie', 'refreshToken=deleted; Max-Age=0; Expires=Thu, 01 Jan 1970 00:00:00 GMT');
...
export const userRegister = async (req,res)=>{
    try {
        ...
        await Promise.all([usernameExists = await User.findOne({username}), emailExists = await User.findOne({email})]);
